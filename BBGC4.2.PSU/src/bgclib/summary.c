...
//printf("vegC=%f\t litrC=%f\t soilC=%f\t totalC=%f\n",summary->vegc,summary->litrc,summary->soilc,summary->totalc);
...
int wsummary(wstate_struct *ws,wflux_struct* wf, summary_struct* summary)
{
	/* Water state summary variables
		...
		//change from ws->soilw to ws->soilWobs;
		summary->daily_soilw = ws->soilWobs;
		summary->daily_snoww = ws->snoww;
		summary->daily_outflow = wf->soilw_outflow;
//printf("BGCsoilw=%f\t obsSoilw=%f\t ET=%f\t Evap=%f\t Trans=%f\n",ws->soilw,ws->soilWobs,et,evap,trans);
		return(!ok);
}
