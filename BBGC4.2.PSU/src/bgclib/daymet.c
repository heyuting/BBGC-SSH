...
int daymet(const metarr_struct* metarr, metvar_struct* metv, wstate_struct* ws, int metday)
{
	...
	*/
	...
	metv->soilTobs = metarr->obs_soilT[metday]; //added by Y.He Oct/29/14
	
	ws->soilWobs = metarr->obs_soilW[metday];   //added by Y.He Oct/29/14

	//printf("%d\t water=%f\t T=%f\n",metday,ws->soilWobs,metv->soilTobs);
