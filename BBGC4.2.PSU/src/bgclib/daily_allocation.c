...
ntemp_struct* nt, wstate_struct* ws, phenology_struct* phen, double naddfrac, int mode) /* mode is MODE_SPINUP or MODE_MODEL summary_struct* summary, */
/*add ws and phen by Y. He to change allocation*/
{
	int ok=1;
	...
	double vswc;//added by Yuting. 06/24/2014
  //double cum_npp;
	woody = epc->woody;
	...
	/*added by Y.He to change allocation after leaf out, but I don't know if these change make physiological sense*/
//	if(phen->remdays_transfer>0)
	{
...
/*	else
	{
		f1 = 1.5 * epc->alloc_frootc_leafc;
		f2 = epc->alloc_crootc_stemc;
                f3 = 1.5 * epc->alloc_newstemc_newleafc;
                f4 = epc->alloc_newlivewoodc_newwoodc;
                g1 = GRPERC;
                g2 = GRPNOW;
                cnl = epc->leaf_cn;
	...
	//cum_npp=summary->cum_npp;

	////////////////////////Y.He. 06/24/2014//////////////////////////////////
	//we can change f1 here by incorporating some information of soil moisture//
	//we need to add an input parameter, like soil moisture in this function  //
	//ws->soilw: kg/m2 --> m3/m3 : soilw/1000kg/m3/soildepth(0.9m) need to change to variable

	/*vswc=ws->soilw/1000/0.9;
  	if(vswc<0.2)
  		f1=100;
	*/

	/////////07.29.14//////change f3 according to Tech Note CLM4 P210/////////////////
  
	//f3=-0.4+2.7/(1+exp(-4*(avail_c*100-0.1)));

        //printf("cum_npp=%f, f3=%f", avail_c, f3);
	//avail_c need to be changed to annual NPP, but I don't think it makes sense...///
	/////////////////////////////////////////////////////////////////////////////////
	...
	allow for the fraction of growth respiration tha2-t is released at the
